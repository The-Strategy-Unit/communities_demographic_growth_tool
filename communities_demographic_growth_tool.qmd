...
icb_hexes <- icb_hexes |>
  dplyr::rename_with(\(x) sub("icb23", "icb22", x))
...
icb_hexmap <- function(.data, fill_col) {
  ...
  .data |>
    ...
    ggplot2::geom_sf(aes(fill = {{ fill_col }}), colour = "grey80") +
    ggplot2::geom_sf(
    ...
    ggplot2::theme(
      ...
      legend.background = element_rect(fill = light_bkg, colour = light_bkg),
      legend.key = element_rect(fill = light_bkg),
      strip.background = element_rect(fill = dark_slate),
...
projected_contacts_by_icb <- contacts_data |>
  tidyr::unnest("data") |>
...
icb_hexes |>
  dplyr::left_join(projected_contacts_by_icb, c("icb22cdh", "icb22nm")) |>
  icb_hexmap(pct_change) +
  ...
  ggplot2::labs(
    title = "Projected % change in total contacts, by age group, 2022/23 - 2042/43",
    subtitle = sub_wrap(
      "This chart shows the projected change within each broad age group for",
      "the 42 ICBs in England"
    ),
    caption = sub_wrap(
      "Sources: NHS England Community Services Dataset (2022-23),",
      "ONS Population projections, ONS OpenGeography"
    )
  ) +
  ggplot2::facet_wrap(dplyr::vars(broad_age_cat), nrow = 2)
